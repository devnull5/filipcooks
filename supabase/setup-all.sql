-- =====================================================================
-- filipcooks.com — ONE-SHOT SETUP
--
-- Paste this whole file into the Supabase SQL Editor and hit Run. It:
--   1. renames the pre-existing unrelated `recipes` table out of the way
--      (to `recipes_old_backup` — nothing is deleted), then
--   2. creates this site's full schema: tables, triggers, RLS, storage.
--
-- Safe to re-run. Generated from schema.sql — if you edit the schema, edit
-- supabase/schema.sql and regenerate rather than editing this file.
-- =====================================================================

-- ---------------------------------------------------------------------
-- STEP 1: move the stale table aside (no-op if it isn't there)
--
-- Identified by having no `slug` column, which this site's schema requires.
-- Renaming rather than dropping keeps the data recoverable.
-- ---------------------------------------------------------------------
...
begin
  if exists (
    ...
    execute 'alter table public.recipes rename to recipes_old_backup';
    raise notice 'Renamed the pre-existing public.recipes to recipes_old_backup.';
  else
    raise notice 'No stale recipes table to move.';
  end if;
end $$;


-- ---------------------------------------------------------------------
-- STEP 2: the real schema
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- STEP 3: confirm it worked. This should list recipes, reviews, profiles.
-- ---------------------------------------------------------------------
